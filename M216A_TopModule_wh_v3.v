...
reg firstFlag, strikeFlag; //badNextFlag;
integer i;
...
always@(posedge clk_i)
	begin
	if(!rst_i)	// Update state if rst_i iis low.
		begin
		state <= next_state;
		end
	end
...
always@(*)
	begin
		...
		if(rst_i)	// Reset state if rst_i is high.
			begin
			for(i = 0; i <= 12; i = i+1)
				begin
				currOccW[i] <= 8'b0;
	...
	case(state)
		...
		S1:	begin	// Check that current program input is valid.
			if((progHeight >= minHeight) & (progHeight <= maxHeight) & (progWidth >= minWidth) & (progWidth <= maxWidth))
				...
				begin
				next_state <= S2;
				//badNextFlag <= 1'b1;	// Get bad input 0,0 at end of simulation. Would need to edit other blocks to output last good input and then wait around for more valid input. Ignore for now.
				end
			end
